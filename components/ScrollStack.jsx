...
const ScrollStack = ({
  ...
  const cardOffsetsRef = useRef([]);  // natural document offsets, cached once
  ...
  // getBoundingClientRect() reflects transforms, so we must read once at rest.
  ...
  useLayoutEffect(() => {
    ...
    // Measure natural positions while transforms are neutral
    cacheOffsets();

    // Recache on resize since card positions change
    const onResize = () => {
      cardsRef.current.forEach(card => { card.style.transform = 'translateZ(0)'; });
  ...
  // Allow child sections to pause/resume Lenis during their own animations
  useEffect(() => {
    const pause  = () => lenisRef.current?.stop();
    const resume = () => lenisRef.current?.start();
    window.addEventListener('scroll-pause',  pause);
    window.addEventListener('scroll-resume', resume);
    return () => {
      window.removeEventListener('scroll-pause',  pause);
      window.removeEventListener('scroll-resume', resume);
    };
  }, []);
